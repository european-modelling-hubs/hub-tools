...
def format_data(df_quantile, 
    ...
    for index, row in df_quantile.iterrows(): 
        ...
        for quantile in quantiles: 
            data_formatted["origin_date"].append(last_date)
...
def generate_baseline_forecast_fullpipeline(truth_data, 
    ...
    for location in truth_data.location.unique(): 
        ...
        # format data 
        df_quantile_formatted = format_data(df_quantile, location=location, target=target_name, last_date=last_date)
...
quantile_baseline_forecasts.to_csv(os.path.join(args.hub_path, f"model-output/{file_name}"), index=False)
